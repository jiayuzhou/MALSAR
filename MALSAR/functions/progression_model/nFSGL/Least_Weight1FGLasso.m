%% FUNCTION Least_Weight1FGLasso
%   Weighted Fused Group Lasso with Least Squares Loss.
%
%% OBJECTIVE
%   argmin_W { \sum_i^t (0.5 * norm (Y{i} - X{i}' * W(:, i))^2)
%              + \sum_i^d rho1{i} * \|W(i, :)\|_1
%              + rho2 * \|W*R\|_1
%          }
% rho1: reweighted Lasso sparse.
% rho2: fused Lasso.
...
function [W, funcVal] = Least_Weight1FGLasso(X, Y, rho1, rho2, opts)
...
% private functions
    ...
    function [Wp] = ReweightFGLasso_projection (W, lambda_1, lambda_2)
        ...
        for i = 1 : size(W, 1)
            ...
            w = flsa(v, w0,  lambda_1(i), lambda_2, length(v), 1000, 1e-9, 1, 6);
    ...
    function [non_smooth_value] = nonsmooth_eval(W, rho_1, rho_2)
        ...
        for i = 1 : size(W, 1)
            ...
            non_smooth_value = non_smooth_value ...
                + rho_1(i) * norm(w, 1) + rho_2 * norm(R * w', 1);
        end
    end
